	...
	<div class="container">
		<nav aria-label="breadcrumb">
			<ol class="breadcrumb p-3">
				...
				<li class="breadcrumb-item active" aria-current="page" data-pt="title">{{ $karir->karir_title }}</li>
			</ol>
		</nav>
	</div>
</section>
		...
		<div class="container pt-0 pt-md-4 pb-4 mb-4 mb-lg-0">
		  <div class="row">
		    <div class="col-lg-8 order-2 order-lg-1 text-white">
		      ...
		      <h2 class="mb-3" data-pt="title">{{ $karir->karir_title }}</h2>
		      <p class="m-0">Oleh :  {{ $karir->nama_user }}<span data-pt="author"></span><br> Terakhir diperbarui pada {{ generate_date($karir->karir_at) }} <span data-pt="date"></span></p>
		    ...
		    <div class="col-lg-4 order-1 order-lg-2 stick sticky-top px-0 px-md-3" style="height:1px">
		      <div class="sticky-sidebar">
		        <div class="card rounded-2 border-0 shadow-sm mb-3 mb-lg-0">
		          <img class="card-img-top" data-pt="image" src="{{ image('assets/images/karir/'.$karir->karir_gambar, 'karir') }}">
		          <div class="card-body d-none d-lg-block">		          	
		            <span class="badge bg-primary mb-2 p-2 text-white">Karir</span>
		            <h5 class="mb-3" data-pt="title">{{ $karir->karir_title }}</h5>
					<p class="mb-1" data-pt="status"></p>
		            <p class="m-0">Oleh : {{ $karir->nama_user }} <span data-pt="author"></span><br> Terakhir diperbarui pada {{ generate_date($karir->karir_at) }} <span data-pt="date"></span></p>
		            <a class="btn btn-primary mt-3 rounded"  href="{{ $karir->karir_url }}">Daftar Karir</a>
		          </div>
		        </div>
		      </div>
		    </div>
