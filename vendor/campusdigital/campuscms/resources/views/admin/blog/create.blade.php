                    ...
                    <div class="tile-body">
                        <form id="form" method="post" action="{{ route('admin.blog.store') }}"
                            ...
                            <div class="form-group row">
                                <label class="col-md-2 col-form-label">Judul Artikel <span
                                        class="text-danger">*</span></label>
                                <div class="col-md-10">
                                    <input type="text" name="judul_artikel"
                                        class="form-control {{ $errors->has('judul_artikel') ? 'is-invalid' : '' }}"
                                        value="{{ old('judul_artikel') }}">
                                    @if ($errors->has('judul_artikel'))
                                        <div class="small text-danger mt-1">{{ ucfirst($errors->first('judul_artikel')) }}
                                        </div>
                                    @endif
                                </div>
                            </div>
                                ...
                                <div class="col-md-10">
                                    <select name="kategori"
                                        ...
                                        @foreach ($kategori as $data)
                                            <option value="{{ $data->id_ka }}"
                                                {{ old('kategori') === $data->id_ka ? 'selected' : '' }}>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-10">
                                    ...
                                    <a class="btn btn-sm btn-secondary btn-image" href="#"><i
                                            class="fa fa-image mr-2"></i>Pilih Gambar...</a>
                                    <br>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-10">
                                    <select name="kontributor"
                                        class="form-control {{ $errors->has('kontributor') ? 'is-invalid' : '' }}">
                                        <option value="" disabled selected>--Pilih--</option>
                                        @foreach ($kontributor as $data)
                                            <option value="{{ $data->id_kontributor }}"
                                                {{ old('kontributor') === $data->id_kontributor ? 'selected' : '' }}>
                                                {{ $data->kontributor }}</option>
                                        @endforeach
                                    </select>
                                    @if ($errors->has('kontributor'))
                                        <div class="small text-danger mt-1">{{ ucfirst($errors->first('kontributor')) }}
                                        </div>
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-12">
                                    <button type="submit" class="btn btn-theme-1"><i
...
@section('js-extra')
    ...
    @include('faturcms::template.admin._js-image', [
        'imageType' => 'blog',
        'croppieWidth' => 640,
        'croppieHeight' => 360,
    ])
...
@section('css-extra')
    ...
    <link rel="stylesheet" type="text/css"
        href="{{ asset('assets/plugins/bootstrap-tagsinput/bootstrap-tagsinput.min.css') }}">
